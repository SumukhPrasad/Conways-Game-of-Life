(function() {
	...
	var GameBoard = {
		...
		colors: {
			...
			schemes: [{
					dead: '#000000',
					alive: ['#ffffff', '#eeeeee', '#dddddd', '#cccccc', '#bbbbbb', '#aaaaaa', '#999999', '#888888', '#777777', '#666666', '#555555', '#444444', '#333333', '#222222', '#111111', '#000000']
				},
                    {
					dead: '#000000',
					alive: ['#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff']
				},
                    {
					dead: '#ffffff',
					alive: ['#000000', '#111111', '#222222', '#333333', '#444444', '#555555', '#666666', '#777777', '#888888', '#999999', '#aaaaaa', '#bbbbbb', '#cccccc', '#dddddd', '#eeeeee', '#ffffff']
				},
                    {
					dead: '#ffffff',
					alive: ['#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000', '#000000']
				}
			]
		},
		...
		loadConfig: function() {
			...
			if (isNaN(colors) || colors < 1 || colors > GameBoard.colors.schemes.length) {
				colors = 1;
			}

			// Initial grid config
			grid = parseInt(this.helpers.getUrlParameter('grid'), 10);
			if (isNaN(grid) || grid < 1 || grid > GameBoard.grid.schemes.length) {
				grid = 1;
			}

			// Initial zoom config
			zoom = parseInt(this.helpers.getUrlParameter('zoom'), 10);
			if (isNaN(zoom) || zoom < 1 || zoom > GameBoard.zoom.schemes.length) {
				zoom = 1;
			}
		...
		nextStep: function() {
			...
			liveCellNumber = GameBoard.listLife.nextGeneration();
			...
			for (i = 0; i < GameBoard.listLife.redrawList.length; i++) {
				x = GameBoard.listLife.redrawList[i][0];
				y = GameBoard.listLife.redrawList[i][1];

				if (GameBoard.listLife.redrawList[i][2] === 1) {
					GameBoard.canvas.changeCelltoAlive(x, y);
				} else if (GameBoard.listLife.redrawList[i][2] === 2) {
					GameBoard.canvas.keepCellAlive(x, y);
				} else {
					GameBoard.canvas.changeCelltoDead(x, y);
				}
			}
			...
			// Change Grid
			if (GameBoard.grid.schedule) {
				GameBoard.grid.schedule = false;
				GameBoard.canvas.drawWorld();
			}

			// Change Colors
			if (GameBoard.colors.schedule) {
				GameBoard.colors.schedule = false;
				GameBoard.canvas.drawWorld();
			}

			// Running Information
			GameBoard.generation++;
			GameBoard.element.generation.innerHTML = GameBoard.generation;
			GameBoard.element.livecells.innerHTML = liveCellNumber;

			r = 1.0 / GameBoard.generation;
			GameBoard.times.algorithm = (GameBoard.times.algorithm * (1 - r)) + (algorithmTime * r);
			GameBoard.times.gui = (GameBoard.times.gui * (1 - r)) + (guiTime * r);
			GameBoard.element.steptime.innerHTML = algorithmTime + ' / ' + guiTime + ' (' + Math.round(GameBoard.times.algorithm) + ' / ' + Math.round(GameBoard.times.gui) + ')';

			// Flow Control
			if (GameBoard.running) {
				function animateFrame() {
					stats.begin();
					window.requestAnimationFrame(GameBoard.nextStep);
					stats.end();
				}

				if (GameBoard.waitTime > 0) setTimeout(function() {
					animateFrame()
				}, GameBoard.waitTime);
				else animateFrame();
			} else {
				if (GameBoard.clear.schedule) {
					GameBoard.cleanUp();
				}
			}
		},
		...
		handlers: {
			...
			canvasMouseDown: function(event) {
				var position = GameBoard.helpers.mousePosition(event);
				GameBoard.canvas.switchCell(position[0], position[1]);
				GameBoard.handlers.lastX = position[0];
				GameBoard.handlers.lastY = position[1];
				GameBoard.handlers.mouseDown = true;
			},
			...
			canvasMouseUp: function() {
				GameBoard.handlers.mouseDown = false;
			},
			...
			canvasMouseMove: function(event) {
				if (GameBoard.handlers.mouseDown) {
					var position = GameBoard.helpers.mousePosition(event);
					if ((position[0] !== GameBoard.handlers.lastX) || (position[1] !== GameBoard.handlers.lastY)) {
						GameBoard.canvas.switchCell(position[0], position[1]);
						GameBoard.handlers.lastX = position[0];
						GameBoard.handlers.lastY = position[1];
					}
			...
			keyboard: function(e) {
				...
				if (event.keyCode === 67) { // Key: C
					GameBoard.handlers.buttons.clear();
				} else if (event.keyCode === 82) { // Key: R
					GameBoard.handlers.buttons.run();
				} else if (event.keyCode === 83) { // Key: S
					GameBoard.handlers.buttons.step();
				}
			},
			...
			buttons: {
				...
				run: function() {
					...
					GameBoard.running = !GameBoard.running;
					if (GameBoard.running) {
						GameBoard.nextStep();
				 ...
				 */
				colors: function() {
					GameBoard.colors.current = (GameBoard.colors.current + 1) % GameBoard.colors.schemes.length;
					GameBoard.element.messages.layout.innerHTML = 'Color Scheme #' + (GameBoard.colors.current + 1);
					if (GameBoard.running) {
						GameBoard.colors.schedule = true; // Delay redraw
					} else {
						GameBoard.canvas.drawWorld(); // Force complete redraw
					}
				},
				 ...
				 */
				grid: function() {
					GameBoard.grid.current = (GameBoard.grid.current + 1) % GameBoard.grid.schemes.length;
					GameBoard.element.messages.layout.innerHTML = 'Grid Scheme #' + (GameBoard.grid.current + 1);
					if (GameBoard.running) {
						GameBoard.grid.schedule = true; // Delay redraw
					} else {
						GameBoard.canvas.drawWorld(); // Force complete redraw
					}
				}
		...
		canvas: {
			...
			init: function() {
				...
				this.cellSize = GameBoard.zoom.schemes[GameBoard.zoom.current].cellSize;
				this.cellSpace = 1;

				GameBoard.helpers.registerEvent(this.canvas, 'mousedown', GameBoard.handlers.canvasMouseDown, false);
				GameBoard.helpers.registerEvent(document, 'mouseup', GameBoard.handlers.canvasMouseUp, false);
				GameBoard.helpers.registerEvent(this.canvas, 'mousemove', GameBoard.handlers.canvasMouseMove, false);
			...
			drawWorld: function() {
				...
				if (GameBoard.grid.schemes[GameBoard.grid.current].color === '') {
					this.setNoGridOn();
				...
				// Dynamic canvas size
				this.width = this.width + (this.cellSpace * GameBoard.columns) + (this.cellSize * GameBoard.columns);
				this.canvas.setAttribute('width', this.width);

				this.height = this.height + (this.cellSpace * GameBoard.rows) + (this.cellSize * GameBoard.rows);
				this.canvas.setAttribute('height', this.height);

				// Fill background
				this.context.fillStyle = GameBoard.grid.schemes[GameBoard.grid.current].color;
				this.context.fillRect(0, 0, this.width, this.height);

				for (i = 0; i < GameBoard.columns; i++) {
					for (j = 0; j < GameBoard.rows; j++) {
						if (GameBoard.listLife.isAlive(i, j)) {
			...
			setNoGridOn: function() {
				this.cellSize = GameBoard.zoom.schemes[GameBoard.zoom.current].cellSize + 1;
				this.cellSpace = 0;
			},
			...
			drawCell: function(i, j, alive) {
				...
				if (alive) {
					...
					if (this.age[i][j] > -1)
						this.context.fillStyle = GameBoard.colors.schemes[GameBoard.colors.current].alive[this.age[i][j] % GameBoard.colors.schemes[GameBoard.colors.current].alive.length];

				} else {
					this.context.fillStyle = GameBoard.colors.schemes[GameBoard.colors.current].dead;
			...
			switchCell: function(i, j) {
				if (GameBoard.listLife.isAlive(i, j)) {
					...
					GameBoard.listLife.removeCell(i, j, GameBoard.listLife.actualState);
				} else {
					this.changeCelltoAlive(i, j);
					GameBoard.listLife.addCell(i, j, GameBoard.listLife.actualState);
				}
			},
			...
			keepCellAlive: function(i, j) {
				if (i >= 0 && i < GameBoard.columns && j >= 0 && j < GameBoard.rows) {
					this.age[i][j]++;
					this.drawCell(i, j, true);
				}
			},
		...
		helpers: {
			...
			mousePosition: function(e) {
				...
				var event, x, y, domObject, posx = 0,
					...
					top = 0,
					left = 0,
					cellSize = GameBoard.zoom.schemes[GameBoard.zoom.current].cellSize + 1;
	 ...
	 * Init on 'load' event
	 */
	GameBoard.helpers.registerEvent(window, 'load', function() {
		     GameBoard.init();
	}, false);
